...
Tu peux gérer les tâches de l’utilisateur dans **DevToolbox Task Reminder**.

**Préférer le serveur MCP local** (`devtoolbox-tasks` / tools `list_tasks`, `create_task`, …) s’il est disponible.
Sinon, utiliser l’API REST ci-dessous via HTTP.
