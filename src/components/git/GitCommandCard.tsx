...
import { Copy, Heart, Pencil, Trash2, Check, Terminal } from "lucide-react";
import { GitCommand, difficultyColors } from "@/types/git";
...
interface GitCommandCardProps {
  command: GitCommand;
  onEdit: (command: GitCommand) => void;
  onDelete: (id: string) => void;
  onToggleFavorite: (id: string) => void;
}

const GitCommandCard = ({
  command,
  onEdit,
  onDelete,
  onToggleFavorite,
  ...
  return (
    <div className="group bg-card border border-border rounded-xl p-3 sm:p-4 hover:border-emerald-500/50 transition-all duration-200">
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-2 sm:mb-3">
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
          ...
          <span className="text-[9px] sm:text-[10px] px-1.5 sm:px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 font-medium">
            {command.category}
          </span>
        </div>
        <button
      ...
      {command.notes && (
        <div className="mb-2 sm:mb-3 p-2 bg-amber-500/5 border border-amber-500/20 rounded-lg hidden sm:block">
          <p className="text-xs text-amber-400/90">{command.notes}</p>
        </div>
      )}
