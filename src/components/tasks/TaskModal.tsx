...
const TaskModal = ({ isOpen, onClose, onSave, editTask }: TaskModalProps) => {
  ...
  const [client, setClient] = useState(""); const [clients, setClients] = useState<string[]>([]); const [newClient, setNewClient] = useState("");
  ...
  useEffect(() => {
    if (!isOpen) return;
    api.get<{ clients: Array<{ name: string }> }>('/tasks/clients/list').then((data) => setClients(data.clients.map((c) => c.name))).catch(() => undefined);
    if (editTask) {
      ...
      setClient(""); setLink(""); setTags([]); setPriority('normal'); setNotificationChannels([]); setReminderDays([]); setReminderDatetime("");
    }
  ...
  const addClient = async () => { const value = newClient.trim(); if (!value) return; try { await api.post('/tasks/clients', { name: value }); setClients((current) => [...new Set([...current, value])].sort()); setClient(value); setNewClient(""); } catch { /* un client existant peut être sélectionné */ } };
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={onClose} /><div className="relative max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-border bg-card shadow-xl">
    ...
    <form onSubmit={submit} className="space-y-5 p-5"><div><Label>Titre de la tâche *</Label><input className={fieldClass + " mt-1.5 text-base"} value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="Ex. Valider la maquette d'accueil" /></div>
      ...
      <div><Label>Client</Label><div className="mt-1.5 flex gap-2"><select className={fieldClass} value={client} onChange={(e) => setClient(e.target.value)}><option value="">Sans client</option>{clients.map((item) => <option key={item} value={item}>{item}</option>)}</select><input className={fieldClass} value={newClient} onChange={(e) => setNewClient(e.target.value)} placeholder="Nouveau client" /><button type="button" onClick={addClient} className="rounded-lg border border-border px-3 hover:bg-muted" title="Ajouter le client"><Plus className="h-4 w-4" /></button></div></div>
      ...
      <div><Label>Canaux de notification pour cette tâche</Label><p className="mb-2 mt-1 text-xs text-muted-foreground">Laissez vide pour utiliser les canaux configurés dans votre compte.</p><div className="flex flex-wrap gap-3">{CHANNELS.map((channel) => <label key={channel.id} className="flex items-center gap-2 text-sm"><Checkbox checked={notificationChannels.includes(channel.id)} onCheckedChange={() => toggle(channel.id)} />{channel.label}</label>)}</div></div>
      ...
      <div><Label>Rappels avant l'échéance</Label><div className="mt-2 flex flex-wrap gap-4">{[7, 3, 1].map((days) => <label key={days} className="flex items-center gap-2 text-sm"><Checkbox checked={reminderDays.includes(days)} onCheckedChange={() => setReminderDays((current) => current.includes(days) ? current.filter((d) => d !== days) : [...current, days].sort((a, b) => b - a))} /><span>{days} jour{days > 1 ? 's' : ''} avant</span></label>)}</div></div>
